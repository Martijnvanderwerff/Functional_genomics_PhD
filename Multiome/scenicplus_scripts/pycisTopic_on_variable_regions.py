...
# Keep only the highly variable regions
# Read variable regions file
with open('variable_regions_10000_mono.txt') as f:
  variable_regions = [region.strip() for region in f.readlines()]

# Create dictionary with regions and index to later subset the fragments matrix for the variable regions
regions_index_dict = dict(zip(region_names, range(0,len(region_names))))
# Subset regions 
region_names_var_regions = set(region_names).intersection(set(variable_regions))
region_data_var_regions = region_data[region_data['name'].isin(variable_regions)]

# Catch indices of regions left to filter the fragment matrices
regions_left = [i for i in regions_index_dict.keys() if i in variable_regions]
# remaining indices
remaining_region_indices = [regions_index_dict.get(i) for i in regions_left]
# Filter matrix
fragment_matrix_variable_regions = fragment_matrix_dummy_filtered[np.array(remaining_region_indices),:]
fragment_matrix_variable_regions_binary = fragment_matrix_dummy_binarized_filtered[np.array(remaining_region_indices),:]


# Initialize cisTopic object
cistopic_obj = CistopicObject(
    fragment_matrix=fragment_matrix_variable_regions,
    binary_matrix=fragment_matrix_variable_regions_binary,
    cell_names=cell_names_dummy,
    region_names=region_names_var_regions,
    region_data=region_data_var_regions,
    cell_data=cell_metadata_dummy,
    path_to_fragments=fragment_path,
    project="Monocytes_subset_10000_20_topics_var_regions"
)
...
models=run_cgs_models_mallet(
    ...
    save_path="/groups/umcg-franke-scrna/tmp03/projects/multiome/ongoing/scenic_plus/pycistopic/monocytes_10000_cells_20_variable_regions",
    mallet_path=mallet_path,
)

print("Finished Mallet LDA")

file_list = glob.glob(os.path.join("/groups/umcg-franke-scrna/tmp03/projects/multiome/ongoing/scenic_plus/pycistopic/monocytes_10000_cells_20_variable_regions", "Topic*.pkl"))
...
model = evaluate_models(
    ...
    select_model = 20,
    return_model = True,
    save='figures_10000_mono/model_evaluation_10000_mono_20_topics_variable_regions.pdf'
)

# Add model to cistopic object 
cistopic_obj.add_LDA_model(model)

with open("/groups/umcg-franke-scrna/tmp03/projects/multiome/ongoing/scenic_plus/pycistopic/monocytes_10000_cells_20_variable_regions/cistopic_obj.pkl", 'wb') as f:
   pickle.dump(cistopic_obj, f)

with open("/groups/umcg-franke-scrna/tmp03/projects/multiome/ongoing/scenic_plus/pycistopic/monocytes_10000_cells_20_variable_regions/cistopic_obj.pkl", "rb") as f:
...
plot_metadata(
    cistopic_obj,
    reduction_name='UMAP',
    variables=['celltype_imputed', 'pycisTopic_leiden_10_0.6', 'pycisTopic_leiden_10_1.2', 'pycisTopic_leiden_10_3'],
    target='cell', num_columns=4,
    text_size=10,
    dot_size=5,
    save='figures_10000_mono/umap_20_topics_variable_regions_celltype_imputed.pdf')
...
plot_topic(
    ...
    target = 'cell',
    num_columns=4,
    save='figures_10000_mono/umap_20_topics_monocytes_colored_by_topics_variable_regions.pdf'
)
...
variable_regions = find_highly_variable_features(
    ...
    n_top_features=None,
    plot=True,
    save='figures_10000_mono/variable_regions_20_topics_10000_monocytes.pdf'
)
...
with open('variable_regions_10000_mono.txt', 'w') as f:
